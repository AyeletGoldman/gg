...
private extension URLQueryEncoder {
    ...
    func append(_ value: String, forKey codingPath: [CodingKey]) {
        ...
        if _explode {
            if codingPath.count > 1 { // Encoding an object
                if _isDeepObject {
                    // We build the parameter like key[path][to][leaf]=value
                    let key = codingPath[0].stringValue
                    let path = codingPath.dropFirst().map { "[\($0.stringValue)]" }.joined()
                    queryItems.append(URLQueryItem(name: "\(key)\(path)", value: value))
                } else {
                    // We build the parameter ignoring all previous paths except the two last ones
                    // eg given a coding path of "path", "to", "leaf", the query item becomes "to[leaf]=value"
                    // but only if the coding paths is > than 2
                    if codingPath.count > 2 {
                        let key = codingPath[codingPath.count - 2].stringValue
                        let path = "[\(codingPath.last!.stringValue)]"
                        queryItems.append(URLQueryItem(name: "\(key)\(path)", value: value))
                    } else {
                        let key = codingPath[codingPath.count - 1].stringValue
