...
public struct ProductColorDisplay {

    private let backingView: any View

    /// Creates a new `ProductColorDisplay` with the given `View`
    /// - Parameter view: the view to wrap in this display
    public init(view: some View) {
        self.backingView = view
    }

    /// Creates a new `ProductColorDisplay` with the given closure
...
public extension ProductColorDisplay {
    /// Creates a new `ProductColorDisplay` with a given hex value. The underlying view is a `Color`.
    /// - Parameter hex: the hex code (with a leading `#`) to use to create a `Color`
    /// - Returns: a `ProductColorDisplay` instance that wraps a `Color` created from the given hex code
    static func color(hex: String) -> ProductColorDisplay {
        return ProductColorDisplay(view: Color(hex: hex))
    }

    /// Creates a new `ProductColorDisplay` with a given image source. The underlying view is a `MaisonKitImage`.
    /// - Parameter source: the source of the image.
    /// - Returns: a `ProductColorDisplay` instance that wraps a `MaisonKitImage` created from the given image source.
    static func image(source: ImageSource) -> ProductColorDisplay {
        return ProductColorDisplay(view: MaisonKitImage(source: source))
    }
}
