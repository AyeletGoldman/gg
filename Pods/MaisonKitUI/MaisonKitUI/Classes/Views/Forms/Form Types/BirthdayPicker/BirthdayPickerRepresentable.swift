...
struct BirthdayPickerRepresentable: UIViewRepresentable {
    @Binding var selection: Birthday

    let labelOptions: [[String]] = BirthdayPickerField.fields.map { $0.getLabelOptions() }

    func makeCoordinator() -> BirthdayPickerRepresentable.Coordinator {
        Coordinator(self)
    }

    func makeUIView(context: UIViewRepresentableContext<BirthdayPickerRepresentable>) -> UIPickerView {
    ...
    func updateUIView(_ view: UIPickerView, context: UIViewRepresentableContext<BirthdayPickerRepresentable>) {
    ...
    final class Coordinator: NSObject, UIPickerViewDataSource, UIPickerViewDelegate {
        var parent: BirthdayPickerRepresentable
        var needsUpdate = true

        init(_ pickerView: BirthdayPickerRepresentable) {
...
struct BirthdayPicker_Previews: PreviewProvider {
    static var previews: some View {
        BirthdayPickerRepresentable(selection: .constant(Birthday(day: 1, month: 2)))
            .background()
